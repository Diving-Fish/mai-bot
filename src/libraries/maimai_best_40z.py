...
class DrawBest(object):
    ...
    def __init__(self, sdBest:BestList, dxBest:BestList, userName:str, playerRating:int, musicRating:int, staticPath='src/static/'):
        self.sdBest = sdBest
        self.dxBest = dxBest
        ...
        self.pic_dir = staticPath + 'mai/pic/'
        self.costom_dir = staticPath + 'mai/custom/'
        self.staticPath = staticPath
        self.cover_dir = staticPath + 'mai/cover/'
        self.img = Image.open(self.pic_dir + 'UI_TTR_BG_Base_Plus.png').convert('RGBA')
        # self.img = Image.new('RGBA', (1500, 1000), 'white')
        self.ROWS_IMG = [2]
        for i in range(6):
    ...
    def _drawBestList(self, img:Image.Image, sdBest:BestList, dxBest:BestList):
        ...
        triangleSize = 27
        levelTriagle = [(itemW, 0), (itemW - triangleSize, 0), (itemW, triangleSize)]
        rankPic = 'D C B BB BBB A AA AAA S Sp SS SSp SSS SSSp'.split(' ')
        comboPic = ' FC FCp AP APp'.split(' ')
        imgDraw = ImageDraw.Draw(img)
        titleFontName = self.staticPath + 'adobe_simhei.otf'
        for num in range(0, len(sdBest)):
            ...
            font = ImageFont.truetype(self.staticPath + 'adobe_simhei.otf', 12, encoding='utf-8')
            tempDraw.text((8, 44), f'Base: {chartInfo.ds} -> {chartInfo.ra}', 'white', font)
            font = ImageFont.truetype(self.staticPath + 'adobe_simhei.otf', 18, encoding='utf-8')
    ...
    def draw(self):
        splashLogo = Image.open(self.costom_dir + 'MAIUI_ZJU.png').convert('RGBA')
        splashLogo = self._resizePic(splashLogo, 0.30)
        self.img.paste(splashLogo, (10, 10), mask=splashLogo.split()[3])

        ratingBaseOffset = 120
        ...
        self.img.paste(ratingBaseImg, (240+ratingBaseOffset, 8), mask=ratingBaseImg.split()[3])
        ...
        font1 = ImageFont.truetype(self.staticPath + 'msyh.ttc', 28, encoding='unic')
        ...
        namePlateImg.paste(nameDxImg, (230+ratingBaseOffset, 4), mask=nameDxImg.split()[3])
        self.img.paste(namePlateImg, (240+ratingBaseOffset, 40), mask=namePlateImg.split()[3])

        shougouImg = Image.open(self.pic_dir + 'UI_CMN_Shougou_Rainbow.png').convert('RGBA')
        shougouDraw = ImageDraw.Draw(shougouImg)
        font2 = ImageFont.truetype(self.staticPath + 'adobe_simhei.otf', 14, encoding='utf-8')
        ...
        self.img.paste(shougouImg, (240+ratingBaseOffset, 83), mask=shougouImg.split()[3])
